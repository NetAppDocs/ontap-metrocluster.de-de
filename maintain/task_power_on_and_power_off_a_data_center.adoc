...
image::power-on-off-data-center.gif[Workflow zum Ausschalten von Cluster_B in der MetroCluster -Konfiguration zur Wartung]
...
.. Überprüfen Sie, ob der lokale und der Remote-Speicher vom Knoten im Wartungsmodus sichtbar sind:
+
`disk show -v`
