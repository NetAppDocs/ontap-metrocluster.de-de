...
---
= ISL-Ports auf Cisco Switches in einer MetroCluster IP-Konfiguration unterbrechungsfrei hinzufügen, entfernen oder ändern
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Wenn Sie einen Cisco 9336C-FX2 Switch in einer Shared-Konfiguration zur Verbindung von NS224 Shelves verwenden, kann eine Änderung der ISLs eine neue Referenz-Konfigurationsdatei (RCF) erfordern. Eine neue RCF ist nicht erforderlich, wenn Ihre aktuelle und neue ISL-Geschwindigkeit 40 Gbit/s und 100 Gbit/s beträgt. Alle anderen Änderungen der ISL-Geschwindigkeit erfordern eine neue RCF. Beispielsweise ist für eine Änderung der ISL-Geschwindigkeit von 40 Gbit/s auf 100 Gbit/s keine neue RCF erforderlich, für eine Änderung von 10 Gbit/s auf 40 Gbit/s hingegen schon.
...
NOTE: Wenn Sie Cisco 9336C-FX2 Switches in einer gemeinsam genutzten Konfiguration zur Verbindung von NS224 Shelves verwenden und das RCF aktualisieren sowie die neue Konfiguration für die neuen ISL-Ports anwenden müssen, sind die folgenden Schritte zu beachten: link:task_upgrade_rcf_files_on_mcc_ip_switches.html["Die RCFs auf MetroCluster IP-Switches aktualisieren."]
