...
---
= Eine einzelne FC-zu-SAS-Brücke in einer MetroCluster FC Konfiguration ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Schließen Sie die FC-Kabel an dieselben Ports an der neuen Bridge an.
. Aktualisieren Sie die FibreBridge-Firmware auf jeder Bridge.
+
Wenn die neue Bridge vom gleichen Typ wie die Partner Bridge ist, aktualisieren Sie auf dieselbe Firmware wie die Partner Bridge. Falls sich die neue Brücke von einem anderen Typ der Partnerbrücke unterscheidet, aktualisieren Sie auf die neueste Firmware, die von der Bridge und Version von ONTAP unterstützt wird. Siehe link:task_update_firmware_on_a_fibrebridge_bridge_parent_topic.html["Firmware auf einer FibreBridge-Bridge aktualisieren"]

. [[step18-reconnect-newbridge]] Schließen Sie die SAS-Kabel wieder an die gleichen Ports der neuen Bridge an.
+
...
. [[step19-verify-each-Bridge]] Stellen Sie sicher, dass jede Bridge alle Festplattenlaufwerke und Festplatten-Shelfs sehen kann, mit denen die Bridge verbunden ist.
+
[cols="35,65"]
|===
...
Das Ist Korrekt
 a| 
Wiederholen Sie den <<step19-verify-each-bridge,Schritt 19>>Vorgang für jede verbleibende Brücke.
...
Nicht richtig
 a| 
.. Überprüfen Sie die SAS-Kabel auf lose Kabel, oder korrigieren Sie die SAS-Verkabelung, indem Sie wiederholen<<step18-reconnect-newbridge,Schritt 18>>.
.. Wiederholen.<<step19-verify-each-bridge,Schritt 19>>
...
. Aktualisieren Sie die FibreBridge-Firmware auf jeder Bridge.
+
link:task_update_firmware_on_a_fibrebridge_bridge_parent_topic.html["Aktualisieren Sie die Firmware auf einer FibreBridge-Bridge"]

. [[step18-reconnect-cables]]Verbinden Sie die SAS-Kabel wieder mit denselben Ports an der neuen Bridge.
+
...
Nicht richtig
 a| 
.. Überprüfen Sie die SAS-Kabel auf lose Kabel, oder korrigieren Sie die SAS-Verkabelung, indem Sie wiederholen<<step18-reconnect-cables,Schritt 18>>.
...
. Schließen Sie die FC-Kabel an die Fibre-Channel-1-Ports der neuen Bridge an.
+
Sie müssen den FC-Port mit demselben Switch oder Controller-Port verkabeln, mit dem auch die FibreBridge 6500N Bridge verbunden war.

. Aktualisieren Sie die FibreBridge-Firmware auf jeder Bridge.
+
Wenn die neue Bridge vom gleichen Typ wie die Partner Bridge ist, aktualisieren Sie auf dieselbe Firmware wie die Partner Bridge. Wenn die neue Bridge einen anderen Typ als die Partner-Bridge hat, aktualisieren Sie auf die neueste Firmware und Version von ONTAP, die von der Bridge unterstützt wird.

+
link:task_update_firmware_on_a_fibrebridge_bridge_parent_topic.html["Aktualisieren Sie die Firmware auf einer FibreBridge-Bridge"]

. [[step18-recable]]Schließen Sie die SAS-Kabel wieder an die SAS A-Ports der neuen Bridge an.
+
...
Nicht richtig
 a| 
.. Überprüfen Sie die SAS-Kabel auf lose Kabel, oder korrigieren Sie die SAS-Verkabelung, indem Sie wiederholen<<step18-recable,Schritt 18>>.
.. Wiederholen Sie den vorherigen Schritt für jede verbleibende Brücke.
...
. Nach dem Austausch des Teils senden Sie das defekte Teil an NetApp zurück, wie in der mit dem Kit gelieferten RMA-Anleitung beschrieben. Weitere Informationen sind auf der link:https://mysupport.netapp.com/site/info/rma["Teilerückgabe  Austausch"^] Seite zu finden.
