...
---
= Einen Switch in einer MetroCluster IP-Konfiguration ersetzen oder anderweitig verwenden
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
CAUTION: Wenn ein Schalteraustausch auf den Plattformen AFF A220, AFF A250 oder FAS2750 erforderlich ist, siehe link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/Cluster_and_HA_communication_issues_during_back-end_switch_replacement["der Knowledge Base Artikel „Cluster- und HA-Kommunikationsprobleme beim Austausch von Backend-Switches“"^].
...
[NOTE]
====
Gemischte Verbindungsgeschwindigkeiten werden auf Plattformen, die eine X60134A Netzwerkkarte verwenden, nicht unterstützt. Ob Ihre Plattform betroffen ist, kann der  https://hwu.netapp.com/["Hardware Universe"^] entnommen werden.

Wenn ein Switch mit 25G-Ports durch einen neuen Switch mit 100G-Ports auf einer betroffenen Plattform ersetzt wird (zum Beispiel ein BES-53248 Switch durch einen Cisco 9336C Switch auf einer AFF A50 Plattform), siehe link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/100G_link_not_coming_online_during_MetroCluster_back-end_switch_replacement?caseId=2010523490["der Knowledge Base Artikel „100G-Link kommt während des MetroCluster Back-End-Switch-Austauschs nicht online“"^].
...
+
image::../media/mcc_ip_hardware_architecture_ip_interconnect.png[MetroCluster IP-Hardwarearchitektur mit Darstellung von Inter-Switch-Verbindungen und Switch-Typen]
...
image:../media/mcc_port_usage_workflow.png["Workflow für MetroCluster IP-Port-Nutzung und Verkabelungsänderungen beim Switch-Austausch"]
...
NOTE: Die Switches sind in folgender Reihenfolge zu aktualisieren: Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. Die Schritte zum Verschieben der Cluster-LIFs auf das zweite Fabric (Switch_A_2, Switch_B_2) sind nach der Konfiguration der Switches im ersten Fabric (Switch_A_1, Switch_B_1) zu wiederholen. Siehe link:../install-ip/task_install_and_cable_the_mcc_components.html["Informationen zur Konfiguration der MetroCluster IP-Switches"].
...
+
Sie können die Schritte in link:../install-ip/using_rcf_generator.html["MetroCluster IP-Switches verkabeln"] ausführen.
...
+
... Die link:https://docs.netapp.com/us-en/ontap-systems-switches/switch-bes-53248/migrate-to-2n-switched.html["Migration zu einer NetApp Cluster-Umgebung mit Switch"^]Vorgehensweise dient dazu, einen Cluster ohne Switches in einen Cluster mit Switches umzuwandeln. Die in link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/using_rcf_generator.html["Installation und Konfiguration von MetroCluster IP"] oder in den RCF-Verkabelungsdateien angegebenen Ports werden verwendet, um die lokale Clusterschnittstelle zu verbinden.
...
+
Wenn der neue Switch identisch ist, kann der neue Switch eingeschaltet werden. Bei einem Upgrade oder Downgrade der Switches können beide Switches eingeschaltet werden. Die Konfiguration kann mit zwei verschiedenen Switches an jedem Standort betrieben werden, bis das zweite Fabric aktualisiert ist.

. Wiederholen Sie die Schritte, um zu überprüfen, ob die MetroCluster-Konfiguration ordnungsgemäß ist <<all_step1,Schritt 1>>.
+
Wenn die Switches im ersten Fabric aktualisiert oder herabgestuft werden, können einige Warnmeldungen im Zusammenhang mit dem lokalen Clustering auftreten.

+

NOTE: Wenn das Fabric aktualisiert oder herabgestuft wird, sind alle Schritte für das zweite Fabric zu wiederholen.

. Alle Cluster-LIFs auf ihren Home Port zurücksetzen.
+

CAUTION: Die Cluster LIFs dürfen erst dann auf ihre Home Ports zurückgesetzt werden, wenn die vorherigen Schritte für alle Switches abgeschlossen sind, die ein Update erfordern.
...
----
.. Alle Cluster-LIFs, die sich derzeit nicht auf ihren Home Ports befinden, manuell auf ihre Home Ports zurücksetzen:
+
