...
---
= Aktivieren Sie die Konsolenprotokollierung vor dem MetroCluster IP-Upgrade
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Prüfen Sie die Informationen inlink:upgrade-mcc-ip-system-controller-replace-set-bootarg.html["Legen Sie den erforderlichen Bootarg fest (für Upgrades auf Systeme, die ab 9.15.1 eingeführt wurden)."], um zu bestätigen, ob Sie einen erforderlichen Bootarg auf dem vorhandenen System festlegen müssen.
