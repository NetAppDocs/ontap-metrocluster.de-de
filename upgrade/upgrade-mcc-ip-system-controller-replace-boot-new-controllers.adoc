...
+
Verwenden Sie die Schritte in link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Legen Sie die MetroCluster-IP-Bootarg-Variablen fest"].

. Überprüfen Sie, ob die Partner-Sysid korrekt ist:
+
`printenv partner-sysid`
...
NOTE: Wenn die Bootmenüoption  `6` ausgeführt wird, müssen die MetroCluster Bootarg-Werte möglicherweise zurückgesetzt werden. Siehe link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Legen Sie die MetroCluster-IP-Bootarg-Variablen fest"].
...
.. Sobald die Intercluster-LIFs aktiv sind, überprüfen Sie den Cluster-Peer-Status und stellen Sie das Cluster-Peering bei Bedarf wieder her.
+
