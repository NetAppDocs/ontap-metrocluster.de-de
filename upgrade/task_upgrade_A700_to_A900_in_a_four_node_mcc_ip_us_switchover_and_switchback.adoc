...
---
= Upgrade von AFF A700 auf AFF A900 oder von FAS9000 auf FAS9500 in einer MetroCluster IP Konfiguration mithilfe von Switchover und Switchback
:allow-uri-read: 


[role="lead"]
Ab ONTAP 9.10.1 kann der MetroCluster Switchover Vorgang genutzt werden, um Clients während der Aktualisierung der Controller-Module im Partnercluster einen unterbrechungsfreien Service bereitzustellen. Andere Komponenten (z. B. Storage Shelfs oder Switches) können im Rahmen dieses Verfahrens nicht aktualisiert werden.
...
** link:../install-ip/task_switch_config_cisco.html#reset-the-cisco-ip-switch-to-factory-defaults["Setzen Sie den Cisco IP-Switch auf die Werkseinstellungen zurück"]
...
** link:../install-ip/task_switch_config_cisco.html#download-and-install-the-cisco-ip-rcf-files["Laden Sie die Cisco IP RCF-Dateien herunter, und installieren Sie sie"]
...
+
Sie benötigen im weiteren Verlauf des Upgrade-Vorgangs auch die aktuellste Passphrase für den Onboard Key Manager.
...
--
Sammeln Sie die externen Schlüsselverwaltungsinformationen, die Sie benötigen, um die Schlüsselmanager-Konfiguration später im Verfahren wiederherzustellen.
...
+
In diesem Beispiel lautet der Name der Admin-SVM cluster_B:
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#remove-metrocluster-configurations["Entfernen von MetroCluster-Konfigurationen"] im _MetroCluster Tiebreaker Installations- und Konfigurationsinhalt_
...
.Bevor Sie beginnen
* Überprüfen Sie, ob Sie mit dem System auf einen HTTP-Server zugreifen können.
...
+
.. Eingeben `n` Um die Wiederherstellung der Sicherung zu überspringen, wenn die folgende Aufforderung angezeigt wird:
+
....
Do you want to restore the backup configuration now? {y|n} n
....
.. Eingeben `y` Starten Sie das System neu, wenn die folgende Aufforderung angezeigt wird:
+
....
...
.Schritte
. Die IP-Switches werden für die Anwendung der neuen Referenzkonfigurationsdateien (RCFs) vorbereitet.
+
Befolgen Sie die Schritte im Abschnitt für Ihren Switch-Anbieter:
...
** link:../install-ip/task_switch_config_cisco.html#download-and-installing-the-cisco-ip-rcf-files["Laden Sie die Cisco IP-RCFs herunter, und installieren Sie sie"]
...
NOTE: Wenn die Bootmenüoption  `6` ausgeführt wird, müssen die MetroCluster Bootarg-Werte möglicherweise zurückgesetzt werden. Siehe link:task_upgrade_A700_to_A900_in_a_four_node_mcc_ip_us_switchover_and_switchback.html#set-the-metrocluster-ip-bootarg-variables["Legen Sie die MetroCluster-IP-Bootarg-Variablen fest"].
