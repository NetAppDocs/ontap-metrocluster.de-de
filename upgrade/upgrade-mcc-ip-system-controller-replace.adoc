...
---
= Workflow zur Aktualisierung des MetroCluster IP Controllers mithilfe des Austauschs des System Controllers
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ab ONTAP 9.13.1 kann dieses geführte, automatisierte MetroCluster Switchover-Verfahren verwendet werden, um ein unterbrechungsfreies Controller-Upgrade in einer MetroCluster IP-Konfiguration mit vier Knoten mithilfe von  `system controller replace` Befehlen durchzuführen. Andere Komponenten (wie Storage Shelves oder Switches) können im Rahmen dieses Verfahrens nicht aktualisiert werden.
