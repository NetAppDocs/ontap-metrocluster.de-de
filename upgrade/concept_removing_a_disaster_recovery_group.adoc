...
NOTE: Diese Schritte verwenden Sie während der Übergangs- und Systemaktualisierungsprozesse. Möglicherweise haben Sie einige Schritte dieses Verfahrens bereits im Rahmen dieser Prozesse durchgeführt.
...
+
image::../media/mcc_dr_groups_8_node.gif[MetroCluster Konfiguration mit acht Knoten in zwei DR-Gruppen]

+
Wenn Sie eine DR-Gruppe entfernen, verbleiben vier Knoten in der Konfiguration.

+
image::../media/mcc_dr_groups_4_node.gif[MetroCluster -Konfiguration mit vier Knoten nach der Entfernung der DR-Gruppe]
...
NOTE: Der `metrocluster remove-dr-group` Befehl wird nur auf ONTAP 9.8 und höher unterstützt.
...
+
Bei einer MetroCluster FC-Konfiguration gehen Sie zum Schritt für <<fc_step_7_remove_nodes,Entfernen der alten DR-Gruppenknoten>>.
...
.. Entfernen Sie die Besitzrechte für die Pool1-Datenträger auf jedem zu löschenden DR-Gruppenknoten. Führen Sie diese Schritte auf jedem zu entfernenden Knoten aus. In den in diesem Schritt verwendeten Beispielen wird node_A_1 entfernt.
+
...
+
Alle Datenträger werden angezeigt, einschließlich der dem Knoten zugeordneten Pool1-Datenträger. Das folgende Beispiel zeigt eine gekürzte Ausgabe, in der Datenträger 0n.4 und seine Partitionen Pool1 zugeordnet sind. Die Ausgabe kann je nach Ihrer Konfiguration variieren.
...
... Entfernen Sie die Festplattenzugehörigkeit für jede Pool1-Festplatte:
+
Wenn Ihre Konfiguration partitionierte Festplatten enthält, entfernen Sie zuerst die Besitzrechte jeder partitionierten Festplatte und anschließend die Besitzrechte der Containerfestplatte:

+
`disk remove_ownership <disk_name>`

+
Im folgenden Beispiel wird die Eigentümerschaft der partitionierten Festplatten entfernt, bevor die Eigentümerschaft der Containerfestplatte `0n.4` entfernt wird:
...
. [[fc_step_7_remove_nodes]]Entferne die Knoten in der alten DR-Gruppe.
+
...
Mehr über die in diesem Verfahren verwendeten Befehle erfahren Sie in der link:https://docs.netapp.com/us-en/ontap-cli/["ONTAP-Befehlsreferenz"^].
