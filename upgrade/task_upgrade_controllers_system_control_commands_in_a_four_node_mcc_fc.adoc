...
---
= Aktualisieren von Controllern mithilfe von system controller replace in einer Vier-Node-MetroCluster FC-Konfiguration
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ab ONTAP 9.10.1 kann diese geführte, automatisierte MetroCluster Switchover-Prozedur verwendet werden, um ein unterbrechungsfreies Controller-Upgrade in einer MetroCluster FC-Konfiguration mit vier Nodes unter Verwendung von  `system controller replace` Befehlen durchzuführen. Andere Komponenten (wie Storage Shelves oder Switches) können nicht als Teil dieser Prozedur aktualisiert werden.
...
* Mit diesem Verfahren ist ein Upgrade von Controllern in einer vier Knoten MetroCluster FC Konfiguration mithilfe von NSO basierter automatisierter Umschaltung und Rückschaltung möglich. Wenn ein Controller Upgrade mittels Aggregate Relocation (ARL) durchgeführt werden soll, siehe link:https://docs.netapp.com/us-en/ontap-systems-upgrade/upgrade-arl-auto-app/["Verwenden Sie „System Controller Replace“-Befehle, um das Upgrade der Controller Hardware mit ONTAP 9.8 oder höher durchzuführen"^]. Die Verwendung des NSO basierten automatisierten Verfahrens wird empfohlen.
