...
NOTE: Wenn die Bootmenüoption  `6` ausgeführt wird, müssen die MetroCluster Bootarg-Werte möglicherweise zurückgesetzt werden. Siehe link:upgrade-mcc-ip-manual-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Legen Sie die MetroCluster-IP-Bootarg-Variablen fest"].
