...
---
= Upgrade von AFF A700 auf AFF A900 oder von FAS9000 auf FAS9500 in einer MetroCluster FC Konfiguration mit Switchover und Switchback
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ab ONTAP 9.10.1 kann der MetroCluster Umschaltvorgang verwendet werden, um Clients während der Aufrüstung der Controller-Module im Partnercluster unterbrechungsfrei bereitzustellen. Andere Komponenten (z. B. Storage Shelfs oder Switches) können im Rahmen dieses Verfahrens nicht aktualisiert werden.
...
image::../media/mcc_upgrade_cluster_a_in_switchover_A900.png[Site_B inaktiv und bereit für ein Upgrade]
...
. Auf jedem Knoten das Systemdatum, die Systemzeit und die Zeitzone prüfen:
+
`show date`

. Gegebenenfalls das Datum in UTC oder Greenwich Mean Time (GMT) festlegen:
+
`set date <mm/dd/yyyy>`

. Die Uhrzeit kann mit folgendem Befehl an der Eingabeaufforderung der Bootumgebung überprüft werden:
+
`show time`

. Gegebenenfalls ist die Zeit in UTC oder GMT festzulegen:
+
`set time <hh:mm:ss>`
...
|===
. Der ha-config-Modus kann überprüft werden:
+
`ha-config show`
...
. Befolgen Sie die folgenden Schritte: link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Hinzufügen von MetroCluster Konfigurationen"] im Abschnitt _MetroCluster Tiebreaker Installation und Konfiguration_.
