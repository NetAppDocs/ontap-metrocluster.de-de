...
.Wichtige Informationen, wenn Sie ein älteres Plattformmodell hinzufügen
...
* Die Tabelle zeigt alle Plattformmodelle, die *gemeinsame MetroCluster/HA-Ports* und *gemeinsame Cluster/HA-Ports* verwenden. Nicht alle Kombinationen werden von diesem Verfahren unterstützt. In der <<expand-ip-supported-platforms,Unterstützte Plattformkombinationen>> kann überprüft werden, ob Ihre Erweiterungskombination unterstützt wird.
...
Die folgenden Tabellen zeigen die unterstützten Plattformkombinationen für Acht-Knoten MetroCluster IP-Konfigurationen.
...
image:../media/expand-ip-group-1-updated.png["Erweiterungskombinationen für AFF und FAS Gruppe-1-Plattformen in MetroCluster IP"]
...
image:../media/expand-ip-group-2-updated.png["Erweiterungskombinationen für AFF und FAS Gruppe-2-Plattformen in MetroCluster IP"]
...
== Entfernen Sie die Konfiguration aus den Überwachungsanwendungen
...
== Bereiten Sie die neuen Controller-Module vor
...
. Im Wartungsmodus geben Sie den Befehl „halt“ ein, um den Wartungsmodus zu verlassen:
+
`halt`

. Prüfen Sie, ob die Partner-Sysid korrekt ist:
+
...
. Starten Sie das System und rufen Sie die Cluster-Einrichtung auf:
+
`boot_ontap`
...
== Konfigurieren Sie Intercluster-LIFs für Cluster-Peering
...
+
Die vollständige Befehlssyntax finden Sie in der link:https://docs.netapp.com/us-en/ontap-cli["ONTAP-Befehlsreferenz"^].
...
+
Im folgenden Beispiel werden die Ports "e0e" und "e0f" der Failover-Gruppe "intercluster01" auf der SVM "cluster01" zugewiesen:
...
+
Das folgende Beispiel zeigt, dass die Intercluster-LIFs "cluster01_icl01" und "cluster01_icl02" auf Port "e0e" auf Port "e0f" ausfallen.
...
+
Sie sollten eine Ausgabe wie im folgenden Beispiel sehen:
...
== Erstellen Sie die MetroCluster-Schnittstellen und spiegeln Sie die Root-Aggregate

Sie müssen die MetroCluster-Schnittstellen auf den neuen MetroCluster-IP-Knoten erstellen und Root-Aggregate spiegeln.
...
.Schritte
. Fügen Sie auf jedem Standort die neuen Intercluster-LIFs zur Peer-Beziehung hinzu.
+
Führen Sie auf cluster_A den folgenden Befehl aus:
...
+
Führen Sie auf cluster_B den folgenden Befehl aus:
...
NOTE: Die Ausgabe des  `metrocluster configuration-settings connection show` Befehls kann je nach Plattformmodell variieren. Beispielsweise werden auf einigen Plattformen die Zeilen für HA Partner nicht in der Ausgabe angezeigt.
...
== Schließen Sie das Hinzufügen der neuen Knoten ab
