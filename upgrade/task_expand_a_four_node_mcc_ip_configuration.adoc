...
Während der Umstellung, Aktualisierung, Installation von acht Knoten und Erweiterung von vier Knoten können Sie Ihrer MetroCluster-Konfiguration ein neues Plattformmodell hinzufügen. Wenn Ihre bestehende MetroCluster-Konfiguration zu irgendeinem Zeitpunkt eine Plattform mit *gemeinsamen Cluster-/HA-Ports* enthält, können Sie der Konfiguration keine Plattform mit *gemeinsamen MetroCluster-/HA-Ports* hinzufügen, ohne Ihre ONTAP-Version auf ONTAP 9.15.1P11 oder ONTAP 9.16.1P4 oder höher zu aktualisieren.
