...
* Die Unterstützung für die Überwachung mehrerer MetroCluster IP-Konfigurationen mit einer einzelnen ONTAP Mediator Instanz hängt davon ab, ob ONTAP Mediator mit HTTPS oder iSCSI konfiguriert wird. Weitere Informationen sind unter link:concept_mediator_requirements.html#mediator-multiple-mccip["ONTAP Mediator zur Überwachung mehrerer MetroCluster IP Konfigurationen verwenden"] zu finden.
