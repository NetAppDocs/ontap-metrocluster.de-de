...
|===
+
** Methode 1:  to upgrade EFOS with downloading the software image to the backup boot partition
** Methode 2:  to upgrade EFOS using the ONIE OS installation
...
NOTE: In seltenen Fällen kann der Switch nicht booten. Folgen Sie den  to upgrade EFOS using the ONIE OS installation Um das neue Image zu installieren.
...
+
.. Konfigurieren Sie die Grundeinstellungen des Switches. Befolgen Sie Schritt 4 und höher:  the Broadcom IP switch to factory defaults
.. Erstellen und wenden Sie die RCF-Datei wie in beschrieben an  and installing the Broadcom RCF files
