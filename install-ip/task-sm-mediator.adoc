...
NOTE: Ein ONTAP Mediator, der HTTPS verwendet, kann nur über die Befehlszeilenschnittstelle (CLI) konfiguriert werden. System Manager wird nicht unterstützt. Weitere Informationen finden Sie unter link:concept_mediator_requirements.html#select-a-communication-protocol-for-ontap-mediator["Wählen Sie ein Kommunikationsprotokoll für ONTAP Mediator aus."].
