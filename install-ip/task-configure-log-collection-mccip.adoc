...
.. SSH-Schlüssel in ONTAP mit  `debug system regenerate-systemshell-key-pair` neu generieren.
.. Die Einrichtung der Protokollerfassung wird mit dem  `system switch ethernet log setup-password` Befehl erneut durchgeführt.
