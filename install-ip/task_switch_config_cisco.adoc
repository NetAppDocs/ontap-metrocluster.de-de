...
---
= Cisco IP-Switches für eine MetroCluster IP Konfiguration konfigurieren
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Wenn diese Schritte im Rahmen eines anderen Verfahrens, wie etwa eines Switch-Austauschs oder einer Switch-Aufrüstung, verwendet werden, sollte geprüft werden, ob die Cluster-LIFs auf den Partner Switch verschoben werden müssen, um einen unterbrechungsfreien Clusterbetrieb zu ermöglichen. link:task_install_and_cable_the_mcc_components.html["Informationen zur Konfiguration der MetroCluster IP-Switches"].
...
.Über diese Aufgabe
Für diese Aufgabe wird eine Dateitransfersoftware wie FTP, TFTP, SFTP, HTTP oder SCP benötigt, um die Dateien auf die Switches zu kopieren.

* Wenn diese Schritte im Rahmen eines anderen Verfahrens, wie etwa eines Switch-Austauschs oder einer Switch-Aufrüstung, verwendet werden, sollte geprüft werden, ob die Cluster-LIFs auf den Partner Switch verschoben werden müssen, um einen unterbrechungsfreien Clusterbetrieb zu ermöglichen. link:task_install_and_cable_the_mcc_components.html["Informationen zur Konfiguration der MetroCluster IP-Switches"].
* Sie müssen die unterstützte Switch-Softwareversion verwenden. Siehe  https://hwu.netapp.com["NetApp Hardware Universe"].
...
. Die vorherigen Schritte werden für die verbleibenden drei IP-Switches in der MetroCluster IP-Konfiguration wiederholt.
+

NOTE: Sie müssen die Switches in folgender Reihenfolge aktualisieren: Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. Falls Ihr Verfahren das Verschieben von Cluster-LIFs erfordert, müssen die Schritte zum Verschieben der Cluster-LIFs auf dem zweiten Fabric (Switch_A_2, Switch_B_2) nach der Konfiguration der Switches im ersten Fabric (Switch_A_1, Switch_B_1) wiederholt werden. Siehe link:task_install_and_cable_the_mcc_components.html["Informationen zur Konfiguration der MetroCluster IP-Switches"].

. Es sollte überprüft werden, ob die automatische Rücksetzung für die Cluster-LIFs aktiviert ist:
+
`network interface show -vserver Cluster -fields auto-revert`

+

CAUTION: Es ist sicherzustellen, dass die automatische Rücksetzung für die Cluster-LIFs aktiviert ist.

. Wenn die automatische Rücksetzung nicht aktiviert ist, sollten alle Cluster-LIFs so geändert werden, dass die automatische Rücksetzung aktiviert ist:
+

NOTE: Wenn Sie diese Schritte im Rahmen eines anderen Vorgangs durchführen, wie etwa eines Switch-Austauschs oder Upgrades, haben Sie möglicherweise die automatische Wiederherstellung im Rahmen der Schritte zu link:switch-move-cluster-lifs.html["Die Cluster-LIFs verschieben"] deaktiviert.
