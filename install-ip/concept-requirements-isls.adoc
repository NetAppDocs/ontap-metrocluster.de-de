...
---
= Anforderungen an MetroCluster IP ISLs
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
== MetroCluster ISL-Anforderungen

Diese Anforderungen gelten für ISLs bei allen MetroCluster IP-Konfigurationen:
...
* Die mindestens erforderliche ISL-Geschwindigkeit hängt vom Plattformmodell ab:
+
** Ab ONTAP 9.18.1 benötigen Plattformen mit einer MetroCluster IP-Backend-Portgeschwindigkeit von 100Gbps eine minimale ISL-Link-Geschwindigkeit von 100Gbps. Die Verwendung einer anderen ISL-Geschwindigkeit erfordert einen Feature Policy Variance Request (FPVR). Um einen FPVR einzureichen, wenden Sie sich an Ihr NetApp Vertriebsteam.
+

CAUTION: Upgrades auf ONTAP 9.18.1 und höher auf Plattformen, die die 100Gbps-ISL-Anforderung derzeit nicht erfüllen, sind nicht blockiert und können durchgeführt werden. Allerdings empfiehlt NetApp Kunden dringend, auf 100Gbps-ISL-Konnektivität umzusteigen, um die erwarteten Leistungs- und Verfügbarkeitsniveaus aufrechtzuerhalten.

** Auf allen anderen Plattformen beträgt die minimal unterstützte ISL-Verbindungsgeschwindigkeit 10 Gbit/s.


* Es muss mindestens ein 10Gbps-ISL-Port pro Fabric vorhanden sein.
