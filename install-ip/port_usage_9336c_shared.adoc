...
---
= Plattform Portzuweisungen für 36 Port Cisco 9336C-FX2 Switches, die NS224 Storage in einer MetroCluster IP-Konfiguration verbinden
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Wenn Sie einen 36-Port Cisco 9336C-FX2 Switch besitzen und nicht planen, NS224 Storage an den Switch anzuschließen, verwenden Sie die Tabellen in link:port_usage_3232c_9336c.html["Plattform-Portzuweisungen für Cisco 3232C- oder 36-Port-Switches Cisco 9336C-FX2"].
...
* Wenn Sie einen Cisco 9336C-FX2 Switch mit NS224 Storage verkabeln, können Sie maximal zwei MetroCluster Konfigurationen oder DR-Gruppen haben. Mindestens eine MetroCluster Konfiguration oder DR-Gruppe muss NS224 Shelves mit dem MetroCluster Switch verbinden.
+
Falls eine Ihrer MetroCluster-Konfigurationen oder DR-Gruppen keine NS224-Shelfs mit dem MetroCluster-Switch verbindet, folgen Sie dem <<tables_not_connecting_ns224,Verkabelungstabellen für Controller, die keine über einen Switch angeschlossenen NS224-Regale verbinden>>.

* Der RcfFileGenerator zeigt nur geeignete Plattformen an, wenn die erste Plattform ausgewählt ist.
* Wenn Sie eine reguläre MetroCluster Konfiguration erweitern, die keinen Switch-gebundenen NS224-Speicher unterstützt, und eine MetroCluster Konfiguration mit Switch-gebundenem NS224-Speicher hinzufügen möchten, beachten Sie die folgenden Informationen:
+
** Im link:https://mysupport.netapp.com/site/tools/tool-eula/rcffilegenerator["RCfFileGenerator"^] Tool müssen Sie zuerst die Plattform auswählen, die Switch-gebundenen NS224-Speicher unterstützt, dann die Plattform, die keinen Switch-gebundenen NS224-Speicher unterstützt.
+
Das bedeutet, dass Sie beim Generieren der RCF für Ihre neue Konfiguration die Plattformen im RcfFileGenerator Tool möglicherweise in der folgenden Reihenfolge auswählen müssen:

+
... Wählen Sie die Plattform aus, die Switch-gebundenen NS224-Speicher aus dem *zweiten Dropdown-Feld* unterstützt.
... Wählen Sie die Plattform aus, die keinen Switch-gebundenen NS224-Speicher aus dem *ersten Dropdown-Feld* unterstützt.
