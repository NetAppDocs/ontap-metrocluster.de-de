...
[role="lead"]
Ermitteln Sie die erforderlichen und unterstützten Hardware- und Softwarekomponenten für eine MetroCluster IP-Konfiguration.  Überprüfen Sie die Namenskonventionen, die in den Dokumentationsbeispielen für die Komponenten verwendet werden.
...
*Hinweis:* 25 GbE Geschwindigkeit erfordert den MAM1Q00A-QSA28 Transceiver-Adapter.
...
NOTE: Die Namen der Aggregate müssen auf allen MetroCluster-Standorten eindeutig sein. Das bedeutet, dass Sie nicht zwei verschiedene Aggregate mit demselben Namen auf Standort A und Standort B haben können.
