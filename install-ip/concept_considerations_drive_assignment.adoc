...
---
= MetroCluster IP-Anforderungen für automatische Antriebszuweisung und ADP-Systeme
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Bei der Verwendung von ADP mit MetroCluster -IP-Konfigurationen sollten Sie Folgendes beachten:
...
Bitte beachten Sie die folgenden Anforderungen, bevor Sie die Tabellen verwenden:

* Sie müssen die MetroCluster Vorgaben hinsichtlich der maximalen Anzahl unterstützter Laufwerke und anderer Richtlinien einhalten. Weitere Informationen finden Sie im https://hwu.netapp.com["NetApp Hardware Universe"] .
* Wenn Sie ein externes Festplattengehäuse wiederverwenden, vergewissern Sie sich, dass die Festplattenzugehörigkeit auf dem externen Festplattengehäuse entfernt wurde, bevor Sie es an den Controller anschließen. Siehelink:https://docs.netapp.com/us-en/ontap/disks-aggregates/remove-ownership-disk-task.html["ONTAP Besitzrechte von einer Festplatte entfernen"^] .
...
=== Festplattenzuordnung auf FAS50-Systemen mit Flash-Cache

Ab ONTAP 9.18.1 wird Flash Cache auf FAS50-Systemen für MetroCluster IP-Konfigurationen unterstützt.

[CAUTION]
====
* Es ist nicht möglich, sowohl Datenaggregate als auch das Stammaggregat mit Flash-Cache-Laufwerken auf dem internen Gehäuse zu betreiben.
* Die Steckplätze 0 und 23 werden für Flash-Cache-Laufwerke verwendet.
* Wenn Sie ein externes Festplattengehäuse wiederverwenden, vergewissern Sie sich, dass die Festplattenzugehörigkeit auf dem externen Festplattengehäuse entfernt wurde, bevor Sie es an den Controller anschließen. Siehelink:https://docs.netapp.com/us-en/ontap/disks-aggregates/remove-ownership-disk-task.html["ONTAP Besitzrechte von einer Festplatte entfernen"] .
...
Keine Angabe
 ...
 a| 
36 Laufwerke (12 interne und 24 externe Laufwerke – die Datenaggregate befinden sich auf dem externen Gehäuse, das Stammaggregat auf dem internen Gehäuse)
 a| 
Die internen Laufwerke sind in vier gleich große Gruppen (Viertel) unterteilt.  Jedes Quartal wird automatisch einem separaten Pool zugeordnet.  Die Laufwerke auf den externen Einschüben sind in vier gleich große Gruppen (Viertel) unterteilt.  Jedes Viertelregal wird automatisch einem separaten Pool zugeordnet.

*Anmerkungen:*

* Wenn Sie interne und externe Laufwerke verwenden, müssen Sie zuerst ONTAP installieren und den lokalen Cluster nur mit den internen Laufwerken konfigurieren.  Anschließend schließen Sie das externe Regal manuell an, nachdem Sie Ihre Installations- oder Einrichtungsaufgabe abgeschlossen haben.
* Für das Root-Aggregat werden mindestens 12 interne Laufwerke benötigt.  Die internen Root-Laufwerke sollten Sie aus Steckplatz 1 einsetzen.  Für beispielsweise 12 interne Laufwerke verwenden Sie die Steckplätze 1-3, 6-8, 12-14 und 18-20.
