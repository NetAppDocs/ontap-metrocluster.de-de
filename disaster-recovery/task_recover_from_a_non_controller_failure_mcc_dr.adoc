...
---
= Wiederherstellung nach einem Nicht-Controller-Ausfall in einer MetroCluster Konfiguration
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Es sollte überprüft werden, dass die LIFs auf den entsprechenden Nodes und Ports gehostet sind, bevor ein Switchback durchgeführt wird.

.Schritte
. Es sollte sichergestellt sein, dass die LIFs auf den entsprechenden Nodes und Ports gehostet sind, bevor ein Switchback durchgeführt wird.
+
...
CAUTION: Vor dem Switchback ist zu überprüfen, ob sich die Daten-LIFs auf den neuen Knoten an der korrekten Position befinden. Beim Switchback der Konfiguration versucht ONTAP, den Datenverkehr über den von den LIFs verwendeten Home-Port wieder aufzunehmen. Ein E/A-Fehler kann auftreten, wenn die Verbindung des Home-Ports zum Switch-Port und zum VLAN nicht korrekt ist. Treten Probleme bei der Platzierung der LIFs auf den richtigen Knoten oder Ports auf, sollte der Switchback nicht fortgesetzt werden und der NetApp Technical Support kontaktiert werden.
...
|===
. Die folgenden Prüfungen bestätigen, dass die Umschaltung zwischen den beiden Clustern betriebsbereit ist:
+
.. Vom DR-Cluster aus prüfen, ob ein Switchover vom überlebenden Cluster zum DR-Cluster möglich ist:
+
`metrocluster switchover -simulate`

.. Vom verbleibenden Cluster aus prüfen, ob ein Wechsel vom DR Cluster zum verbleibenden Cluster möglich ist:
+
`metrocluster switchover -simulate`
...
NOTE: In einer MetroCluster FC-Umgebung sollte 'connections' in der Befehlsausgabe als "not-applicable" angezeigt werden.
