...
image::../media/workflow_high_level_node_and_cluster_configuration_software.gif[Workflow für die Knoten- und Clusterkonfiguration auf hoher Ebene]
...
Beispiel, das in diesem Verfahren verwendet wird: site_B
 a| 
...
Beispiel, das in diesem Verfahren verwendet wird: controller_B_1
 a| 
 a| 
 a| 
 a| 
...
Beispiel, das in diesem Verfahren verwendet wird: controller_B_2
 a| 
 a| 
 a| 
 a| 
...
[CAUTION]
====
* In Fabric OS 10.x und höher unterstützt Brocade für die SNMP-Authentifizierung nur SHA-512. ONTAP unterstützt derzeit kein SHA-512, die Unterstützung ist auf SHA-256 beschränkt.
* Wenn auf Ihren MetroCluster FC Switches Fabric OS 10.x oder höher ausgeführt wird, ist die Konfiguration von SNMP nicht möglich und diese Switches können nicht zur ONTAP Zustandsüberwachung hinzugefügt werden. Wenn ein Downgrade der Switch-Firmware gewünscht ist, können die Schritte in link:../maintain/task_upgrade_or_downgrad_the_firmware_on_a_brocade_fc_switch_mcc.html["Upgrade oder Downgrade der Firmware auf einem Brocade FC-Switch"] befolgt werden.
...
--
.Brocade -Switches (FOS 8.x und früher)
--
`snmpconfig --set snmpv3`

Das Beispiel zeigt, wie ein schreibgeschützter Benutzer konfiguriert wird.  Sie können die RW-Benutzer bei Bedarf anpassen.  Wenn Sie RO-Zugriff benötigen, geben Sie nach „Benutzer (ro):“ den „snmpv3user“ an.
...
NOTE: Sie sollten auch Passwörter für nicht verwendete Konten festlegen, um diese zu sichern und die beste in Ihrer ONTAP-Version verfügbare Verschlüsselung zu verwenden.
