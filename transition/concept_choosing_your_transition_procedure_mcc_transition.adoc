...
---
= MetroCluster FC-zu-IP-Übergangsverfahren Optionen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Unterbrechungsfrei
 a| 
Auf den neuen Bahnsteigen werden neue Lagerregale benötigt.  Nach Abschluss der Umstellung werden die alten Controller, Gehäuse und Festplatten aus dem Cluster entfernt.
 a| 
link:concept_nondisruptively_transitioning_from_a_four_node_mcc_fc_to_a_mcc_ip_configuration.html["Link zum Verfahren"]

*Hinweis:* Dieses Verfahren unterstützt die folgenden FC-zu-IP-Übergänge:

* Von einer MetroCluster -FC-Konfiguration mit vier Knoten zu einer MetroCluster IP-Konfiguration mit vier Knoten
* Von einer MetroCluster -FC-Konfiguration mit acht Knoten zu einer MetroCluster IP-Konfiguration mit acht Knoten
