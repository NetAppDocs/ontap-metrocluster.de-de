...
---
= Verschieben von Linux iSCSI-Hosts von MetroCluster-FC- zu MetroCluster-IP-Knoten
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
In diesem Verfahren werden IPv4-Schnittstellen erstellt.

Die in diesen Verfahren angegebenen Host-Befehle und Beispiele sind für LINUX-Betriebssysteme spezifisch.



== Einrichten neuer iSCSI-Verbindungen

[role="lead"]
Zum Verschieben von iSCSI-Verbindungen müssen Sie neue iSCSI-Verbindungen zu den MetroCluster-IP-Knoten einrichten.

. Erstellen Sie iSCSI-Schnittstellen auf den MetroCluster IP-Knoten, und überprüfen Sie Ping-Verbindungen von den iSCSI-Clients zu den neuen IP-Schnittstellen auf den MetroCluster IP-Knoten.
+
https://docs.netapp.com/us-en/ontap/networking/create_a_lif.html["Erstellen von Netzwerkschnittstellen"^]

+
Alle iSCSI-Schnittstellen der SVM sollten über den iSCSI-Client erreichbar sein.

. Identifizieren Sie auf dem iSCSI-Host oder -Client die vorhandenen iSCSI-Verbindungen vom Host zum MetroCluster-FC-Node: `iscsiadm -m session`
+
[listing]
----
...
----
. Überprüfen Sie die Verbindungen vom MetroCluster-IP-Knoten: `iscsi session show -vserver svm-name`
+
[listing]
----
...
----
. Liste der iscsi-Schnittstellen in ONTAP für die SVM, die die Schnittstellen enthalten: `iscsi interface show -vserver svm-name`
+
[listing]
----
...
----
. Führen Sie auf dem iSCSI-Client die Erkennung auf einer der iSCSI-IP-Adressen der SVM durch, um die neuen Ziele zu ermitteln: `iscsiadm -m discovery -t sendtargets -p iscsi-ip-address`
+
...
----
. Melden Sie sich auf dem iSCSI-Client bei allen erkannten Adressen an: `iscsiadm -m node -L all -T node-address -p portal-address -l`
+
[listing]
----
...
----
. Überprüfen Sie auf dem iSCSI-Client die Anmeldung und die Verbindungen: `iscsiadm -m session`
+
[listing]
----
[root@scspr1789621001 ~]# iscsiadm -m session
...
----
. Überprüfen Sie vom MetroCluster-Knoten die Anmeldung und die Verbindung mit dem Client: `iscsi initiator show -vserver vsa_1`
+
[listing]
----
...
Am Ende dieser Aufgaben kann der Client alle iSCSI-Schnittstellen (auf den MetroCluster-FC- und MetroCluster-IP-Nodes) sehen und ist bei allen diesen Schnittstellen angemeldet.

LUNs und Volumes werden weiterhin physisch auf FC-Nodes gehostet. Da LUNs nur auf MetroCluster FC Node-Schnittstellen gemeldet werden, zeigt der Client nur Pfade über MetroCluster FC Nodes an. Dies ist in den Ausgaben von sanlun lun show -p und Multipath -ll -d zu sehen. Im nächsten Schritt werden IP-Nodes als Reporting-Nodes hinzugefügt.
...
== Hinzufügen der MetroCluster IP-Nodes als Reporting-Nodes

[role="lead"]
Nachdem Sie die Verbindungen zu den neuen MetroCluster IP-Nodes eingerichtet haben, müssen Sie neue Reporting-Nodes hinzufügen.

. Führen Sie auf dem MetroCluster-Node die Reporting-Nodes für LUNs auf der SVM aus: `lun mapping show -vserver vsa_1 -fields reporting-nodes -ostype linux`
+
Die folgenden Knoten sind lokale Nodes, da sich LUNs physisch auf FC Nodes Node_A_1-FC und Node_A_2-FC befinden.
...
----
. Fügen Sie auf dem MetroCluster-Node Reporting-Nodes hinzu: `lun mapping add-reporting-nodes -vserver svm-name -path /vol/vsa_1_vol*/lun_linux_* -nodes node1,node2 -igroup igroup_linux`
+
[listing]
----
...
----
. Überprüfen Sie auf dem MetroCluster-Node, ob die neu hinzugefügten Nodes vorhanden sind: `lun mapping show -vserver svm-name -fields reporting-nodes -ostype linux vserver path igroup reporting-nodes`
+
[listing]
----
...
12 entries were displayed.
----
. Geben Sie am Host den folgenden Befehl ein, um den scsi-Bus erneut auf dem Host zu lesen und die neu hinzugefügten Pfade zu entdecken:``/usr/bin/rescan-scsi-bus.sh -a``
+
[listing]
----
...
0 device(s) removed.
----
. Geben Sie auf dem Host den folgenden Befehl ein, um die neu hinzugefügten Pfade aufzulisten: `sanlun lun show -p`
+
...
----
. Verschieben Sie auf der MetroCluster die Volumes/Volumes, die LUNs enthalten, von FC- auf IP-Nodes.
+
[listing]
----
...
----
. Wenn die Verschiebung des Volumes abgeschlossen ist, können Sie im MetroCluster mit dem Befehl „Volume show“ bestätigen, dass das Volume oder die LUN online sind.
. Die iSCSI-Schnittstellen auf den MetroCluster IP-Knoten, in denen sich die LUN jetzt befindet, werden als primäre Pfade aktualisiert. Wenn der primäre Pfad nach der Volume-Verschiebung nicht aktualisiert wird, führen Sie /usr/bin/rescan-scsi-bus.sh -A und Multipath -v3 aus oder warten Sie einfach, bis Multipath-Rescanning stattfindet.
+
Im folgenden Beispiel ist der primäre Pfad ein LIF auf dem MetroCluster IP Node.
...
== Entfernen von Reporting-Knoten und Rescanning-Pfaden

[role="lead"]
Sie müssen die Berichtsknoten entfernen und die Pfade erneut scannen.

. Entfernen Sie vom MetroCluster IP-Knoten die Remote Reporting Nodes (die MetroCluster IP-Knoten) für die Linux-LUNs: `lun mapping remove-reporting-nodes -vserver vsa_1 -path * -igroup igroup_linux -remote-nodes true`
+
In diesem Fall sind die Remote-Nodes FC-Nodes.
...
12 entries were acted on.
----
. Überprüfen Sie über den MetroCluster IP-Node die Reporting-Nodes für die LUNs: `lun mapping show -vserver vsa_1 -fields reporting-nodes -ostype linux`
+
[listing]
----
node_A_1-IP::*> lun mapping show -vserver vsa_1 -fields reporting-nodes -ostype linux
...
12 entries were displayed.
----
. Prüfen Sie auf dem iSCSI-Host den scsi-Bus erneut: `/usr/bin/rescan-scsi-bus.sh -r`
+
Die Pfade, die entfernt werden, sind die Pfade von FC Nodes.
...
.
----
. Vergewissern Sie sich auf dem iSCSI-Host, dass nur Pfade von den MetroCluster IP-Knoten sichtbar sind: `sanlun lun show -p``multipath -ll -d`
