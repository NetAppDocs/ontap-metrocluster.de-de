...
----
. Führen Sie die iSCSI-Schnittstellen in ONTAP für die SVM auf, die die Schnittstellen enthält: `iscsi interface show -vserver svm-name`
+
[listing]
----
...
. Geben Sie auf dem Host den folgenden Befehl ein, um den SCSI-Bus auf dem Host erneut zu scannen und die neu hinzugefügten Pfade zu ermitteln: `/usr/bin/rescan-scsi-bus.sh -a`
+
[listing]
----
...
----
. Überprüfen Sie das `sg3-utils` Das Paket wird auf dem Linux-Host installiert. Dies vermeidet ein `rescan-scsi-bus.sh utility not found` Fehler beim erneuten Scannen des Linux-Hosts für die neu zugeordneten LUNs mithilfe des `rescan-scsi-bus` Befehl.
. Scannen Sie auf dem iSCSI-Host den SCSI-Bus erneut: `/usr/bin/rescan-scsi-bus.sh -r`
+
...
.
----
. Vergewissern Sie sich auf dem iSCSI-Host, dass nur Pfade von den MetroCluster IP-Knoten sichtbar sind:
+
`sanlun lun show -p`
