...
LUNs und Volumes befinden sich weiterhin physisch auf den alten Knoten. Da LUNs nur über die Schnittstellen der alten Knoten gemeldet werden, zeigt der Host nur Pfade über die alten Knoten an. Um dies zu sehen, führen Sie die `sanlun lun show -p` und `multipath -ll -d` Befehle auf dem Host aus und analysieren Sie die Befehlsausgaben.
