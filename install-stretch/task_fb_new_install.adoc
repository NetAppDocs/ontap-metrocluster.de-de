...
---
= FC-zu-SAS-Brücken und SAS-Festplattengehäuse für eine Stretch MetroCluster Konfiguration installieren
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Installieren und verkabeln Sie ATTO FibreBridge Bridges und SAS-Platten-Shelfs, wenn Sie der Konfiguration neuen Speicher hinzufügen.
...
image::../media/workflow_bridge_installation_and_configuration.gif[Arbeitsablauf für die Installation und Konfiguration der FC-zu-SAS-Brücke]
...
Bei der bandinternen Verwaltung können die Bridges über die ONTAP-CLI über die FC-Verbindung zur Bridge verwaltet und überwacht werden. Der physische Zugriff auf die Bridge über die Ethernet-Ports der Bridge ist nicht erforderlich, wodurch die Sicherheitslücke der Bridge verringert wird.
...
* Vor ONTAP 9.5 wird nur die bandexterne SNMP-Verwaltung unterstützt.


Bridge-CLI-Befehle können über den ONTAP-Schnittstellenbefehl über die ONTAP-Schnittstelle ausgegeben `storage bridge run-cli -name <bridge_name> -command <bridge_command_name>` werden.
...
+
Im können Sie im https://mysupport.netapp.com/matrix["NetApp Interoperabilitäts-Matrix-Tool (IMT)"] Feld *Speicherlösung* Ihre MetroCluster-Lösung auswählen. Mit dem *Komponenten-Explorer* können Sie die Komponenten und die ONTAP-Version auswählen, um Ihre Suche zu verfeinern. Sie können *Ergebnisse anzeigen* auswählen, um die Liste der unterstützten Konfigurationen anzuzeigen, die den Kriterien entsprechen.

* Jeder FC-Controller muss über einen FC-Port verfügen, damit eine Bridge angeschlossen werden kann.
* Sie müssen mit der Handhabung von SAS-Kabeln und den Überlegungen und Best Practices für die Installation und Verkabelung von Festplatten-Shelfs vertraut sein.
+
...
* Die Anforderungen an die Systemkonnektivität für maximale Entfernungen für Platten-Shelves, FC-Controller und Backup-Bandgeräte mit Multimode-Glasfaserkabeln von 50 Mikron gelten auch für FibreBridge Bridges.
+
...
image::../media/mcc_cabling_bridge_and_sas3_stack_with_7500n_and_multiple_stacks.gif[MetroCluster -Verkabelung FibreBridge-Konfiguration mit mehreren SAS3-Stacks]
...
=== Überprüfen Sie die Bridge-Konnektivität und verkabeln Sie die FC-zu-SAS-Brücken mit den Controller-FC-Ports

Sie müssen die Bridges in einer über Bridge verbundenen MetroCluster-Konfiguration mit den Controller-FC-Ports verkabeln.

.Schritte
. [[step1-verify-detect]]Vergewissern Sie sich, dass jede Bridge alle Festplattenlaufwerke und Festplatten-Shelfs erkennen kann, mit denen die Bridge verbunden ist:
+
`sastargets`
...
----
. Vergewissern Sie sich, dass die Befehlsausgabe anzeigt, dass die Bridge mit den richtigen Festplatten und Festplatten-Shelfs im Stack verbunden ist.
+
[cols="30,70"]
|===
...
|===
. [[step3-cable-each-Bridge]]Verkabeln Sie jede Brücke mit den FC-Ports des Controllers:
+
.. Verkabeln Sie den FC-Port 1 der Bridge mit einem FC-Port am Controller in „Cluster_A“
.. Verkabeln Sie den FC-Port 2 der Bridge mit einem FC-Port am Controller in „Cluster_B.“
+
*** Wenn der Controller mit einem FC-Adapter mit vier Ports konfiguriert ist, stellen Sie sicher, dass die Bridges an beiden Enden des Storage-Stacks nicht mit zwei FC-Ports auf demselben ASIC verbunden sind. Beispiel:
+
**** Port A und Port B teilen sich denselben ASIC.
**** Port c und Port d teilen sich denselben ASIC.
+
Verbinden Sie in diesem Beispiel FC_Bridge_A_1 mit Port A und FC_Bridge_A2 mit Port c.



*** Wenn der Controller mit mehr als einem FC-Adapter konfiguriert ist, verkabeln Sie die Brücken an einem der Enden des Storage Stacks nicht mit demselben Adapter.
+
In diesem Szenario sollten Sie FC_Bridge_A_1 mit einem integrierten FC-Port verbinden und FC_Bridge_A_2 mit einem FC-Port an einem Adapter in einem Erweiterungssteckplatz verbinden.

+
image:../media/cluster_peering_network.png["Abbildung eines Cluster-Peering-Netzwerks"]
