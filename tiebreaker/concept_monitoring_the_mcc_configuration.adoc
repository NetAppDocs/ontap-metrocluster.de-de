...
---
= Die MetroCluster Konfiguration mit MetroCluster Tiebreaker überwachen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
== Beobachtermodus für Tiebreaker deaktivieren

Sie können den Beobachtermodus der Tiebreaker-Software deaktivieren, sodass diese nach Erkennung eines Standortausfalls automatisch eine Umschaltung einleitet.


NOTE: In Tiebreaker 1.6 und höher können Sie die automatische Umschaltung beim Hinzufügen eines Monitors mithilfe der Funktion aktivieren. `monitor add wizard` Befehl. Siehelink:concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Fügen Sie MetroCluster-Konfigurationen hinzu"] .

.Schritt
. Beobachtermodus deaktivieren:
+
