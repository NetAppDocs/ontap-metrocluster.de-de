...
| Unterstützte Funktionen in der MetroCluster-Konfiguration | Beschreibung und wo man mehr erfahren kann | Verfügbar ab 
 ...
 a| 
SnapMirror Cloud-Unterstützung für MetroCluster FlexGroup Volumes
 a| 
...
link:https://docs.netapp.com/us-en/ontap/data-protection/cloud-backup-with-snapmirror-task.html#delete-a-snapmirror-cloud-relationship["Sichern Sie Daten in der Cloud mit ONTAP SnapMirror"^]
 a| 
ONTAP 9.18.1GA
 ...
 a| 
Neue unterstützte Upgrade-Kombinationen für MetroCluster IP-Controller-Upgrades mit  `system controller replace` Befehlen
 a| 
Fügt Unterstützung für die Aufrüstung eines AFF A70 auf ein AFF A90 System oder eines FAS70 auf ein FAS90 System mithilfe von  `system controller replace` Befehlen in einer MetroCluster IP-Konfiguration hinzu.

link:../upgrade/upgrade-mcc-ip-system-controller-replace.html["Controller-Upgrades in MetroCluster IP mit vier Nodes mithilfe von Switchover und Switchback mit Befehlen, die den System-Controller ersetzen (ONTAP 9.13.1 und höher)"]
 a| 
ONTAP 9.18.1GA
 ...
 a| 
Flash-Cache-Unterstützung für FAS50-Systeme in MetroCluster -IP-Konfigurationen
 a| 
Flash Cache wird auf FAS50-Systemen in MetroCluster IP-Konfigurationen unterstützt.

link:../install-ip/concept_considerations_drive_assignment.html#disk-assignment-on-fas50-systems-with-flash-cache["Festplattenzuordnung auf FAS50-Systemen mit Flash-Cache"]
 a| 
ONTAP 9.18.1
...
** FlexVol volume pro Cluster: 5000
** SVM-Limits: 64 SVMs pro Cluster
** LIF-Anzahl: 256 LIFs pro Cluster
...
Weitere Informationen finden Sie im link:https://hwu.netapp.com["Hardware Universe"^] für weitere Informationen.
 a| 
ONTAP 9.17.1
...
FibreBridge-Firmware-Update mit Anmeldeinformationen
 a| 
Sie können die Firmware auf FibreBridge-Brücken mithilfe von Anmeldeinformationen aktualisieren, wenn diese vom Server zum Herunterladen des Firmware-Pakets benötigt werden.
...
MetroCluster IP-Unterstützung für End-to-End-Verschlüsselung
 a| 
Die Ende-zu-Ende-Verschlüsselung wird auf den Systemen AFF A400, AFF C400, FAS8300 und FAS8700 unterstützt, um den Backend-Datenverkehr, wie z. B. NVlog- und Speicherreplikationsdaten, zwischen den Standorten in einer MetroCluster IP-Konfiguration zu verschlüsseln.
